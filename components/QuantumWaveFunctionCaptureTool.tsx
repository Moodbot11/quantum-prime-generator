...
const QuantumWaveFunctionCaptureTool: React.FC<Props> = () => {
  ...
  const [outputDigits, setOutputDigits] = useState<number>(10000)
  ...
  const handleCapture = () => {
    ...
    try {
      ...
      // Pad with zeros if necessary to always have 10,000 digits
      formattedResult = formattedResult.padEnd(10000, '0')
  ...
  return (
    <div className="p-6 rounded-lg shadow-lg mx-auto bg-white text-black">
      <h2 className="text-2xl font-bold mb-4 text-cyan-400">Quantum Wave Function Capture Tool (up to 10000 digits input/output)</h2>
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder="Enter frequency (up to 10000 digits)"
          />
        </div>
        <div>
        ...
        <div>
          <label htmlFor="outputDigits" className="block text-sm font-medium">Output Digits (1000-10000)</label>
          <input
            type="number"
            id="outputDigits"
            value={outputDigits}
            onChange={(e) => setOutputDigits(Math.min(10000, Math.max(1000, parseInt(e.target.value) || 1000)))}
            className="mt-1 block w-full rounded-md bg-gray-100 border-gray-300 text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            min="1000"
            max="10000"
          />
